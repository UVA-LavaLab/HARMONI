...
#SBATCH --output=log_slurm/small_%A_%a.out
#SBATCH --error=log_slurm/small_%A_%a.err
#SBATCH --mem=16GB
#SBATCH --cpus-per-task=4
#SBATCH --mail-type=begin,end
#SBATCH --mail-user=vyn9mp@virginia.edu
#SBATCH --array=0-29 # Adjust the range based on the number of lines in params/*.txt
...
while IFS= read -r LINE; do
  ...
  PARAMS+=("$LINE")
done < params/ispass_ae_small.txt
